...
interface DynamicFormProps {
  ...
  // 自定义渲染
  customRender?: Record<string, (field: any, form: FormInstance) => React.ReactNode>;
  ...
  const renderFormField = (field: any): React.ReactNode => {
    ...
    switch (type) {
      ...
      case 'upload':
        ...
        return (
          <Upload {...uploadProps}>
            <Button icon={<UploadOutlined />}>{props.buttonText || '选择文件'}</Button>
          </Upload>
        );
  ...
  const renderFormItem = (field: any) => {
    ...
    return (
      <Form.Item
        ...
        extra={field.extra}
        hidden={field.hidden}
        valuePropName={field.type === 'checkbox' && !field.props?.mode ? 'checked' : 'value'}
      >
