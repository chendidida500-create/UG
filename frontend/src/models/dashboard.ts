...
export default function useDashboardModel (): DashboardModelState
{
  ...
  const getChartData = useCallback( async ( type: string, timeRange?: string ) =>
  {
    ...
    } catch ( error: any )
    {
      ...
      if ( type === 'userTrend' )
      {
        ...
        for ( let i = 0; i < 7; i++ )
        {
          const dateValue = new Date( Date.now() - ( 6 - i ) * 24 * 60 * 60 * 1000 );
  ...
  const getOnlineUsers = useCallback( async () =>
  {
    try
    {
      const response = await request<ApiResponse<any[]>>( '/api/dashboard/online-users', {
        method: 'GET',
      } );
  ...
  const exportReport = useCallback( async ( type: string, timeRange?: string ) =>
  {
    try
    {
      const response = await request<ApiResponse<Blob>>( '/api/dashboard/export', {
        method: 'GET',
        params: { type, timeRange },
        responseType: 'blob',
      } );
