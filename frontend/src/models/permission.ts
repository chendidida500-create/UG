...
export interface PermissionModelState
{
  ...
  batchDeletePermissions: ( ids: string[] ) => Promise<boolean>;
  ...
  filterMenuByPermission: ( menus: unknown[] ) => unknown[];
}
...
export default function usePermissionModel (): PermissionModelState
{
  ...
  const getPermissionList = useCallback(
    async ( params?: Record<string, unknown> ) =>
    {
      ...
      try
      {
        const response = await request<ApiResponse<Permission[]>>(
          '/api/permissions',
          {
            method: 'GET',
            params,
      ...
      } catch ( error: unknown )
      {
        ...
        message.error( apiError.message || '获取权限列表失败' );
        throw error;
      } finally
      {
        setLoading( false );
  ...
  const getAllPermissions = useCallback( async () =>
  {
    ...
    {
      const apiError = error as ApiError;
      message.error( apiError.message || '获取权限树失败' );
      throw error;
    }
  }, [] );
  ...
  const createPermission = useCallback( async ( data: Partial<Permission> ) =>
  {
    ...
    {
      const apiError = error as ApiError;
      message.error( apiError.message || '权限创建失败' );
      throw error;
    }
  }, [] );
  ...
  const updatePermission = useCallback(
    async ( id: string, data: Partial<Permission> ) =>
    {
      ...
      {
        const apiError = error as ApiError;
        message.error( apiError.message || '权限更新失败' );
        throw error;
      }
    },
    []
  ...
  const deletePermission = useCallback( async ( id: string ) =>
  {
    ...
    {
      const apiError = error as ApiError;
      message.error( apiError.message || '权限删除失败' );
      throw error;
    }
  }, [] );
  ...
  const updatePermissionStatus = useCallback(
    async ( id: string, status: 'active' | 'inactive' ) =>
    {
      ...
      {
        const apiError = error as ApiError;
        message.error( apiError.message || '状态更新失败' );
        throw error;
      }
    },
    []
