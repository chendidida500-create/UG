...
## 项目概述
基于 UMI 4.x + React 18 + Ant Design 5.x + Egg.js 3.x + MySQL 8.0 的现代化后台管理系统，实现了完整的RBAC权限管理功能。
...
- **JWT Token认证**: Access Token + Refresh Token双令牌机制
- **密码安全**: bcrypt加密，支持密码重置
- **会话管理**: 自动刷新token，登录状态持久化
- **多端登录**: 支持记住密码，登录历史记录
